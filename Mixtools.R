...
### Univariate symmetric, location-shifted semiparametric example
plot(wait1, which=2, cex.axis=1.4, cex.lab=1.4, cex.main=1.8,
     main2="Time between Old Faithful eruptions", xlab2="Minutes")
wait2<- spEMsymloc(waiting, mu0=c(55,80))
plot(wait2, lty=2, newplot=F, addlegend = F)
...
plot(wait2a, lty=1, addlegend = F, cex.axis=1.4, cex.lab=1.4,
     cex.main=1.8, xlab="Minutes", title="Time between Old Faithful eruptions")

plot(wait2b, lty=2, newplot=F, addlegend = F)
...
lambda<- c(0.4,0.6)
mu<- matrix(c(0,0,0,3,4,5), m, r, byrow = T)
sigma<- matrix(rep(1,6), m, r, byrow = T)

centers<- matrix(c(0,0,0,4,4,4), 2,3, byrow = T)
...
#generate pseudorandom sample of size n=300
n<- 300; z<- sample(m,n,rep=T, prob = lambda)
r1<- 3; z2<- rep(z,r1)
...
centers<- matrix(c(0,0,0,1/2,1/2,4,4,4,1/2,1/2),m,r,byrow = T)
b<-npEM(x,centers,id,eps = 1e-8, verb=F, samebw = F)
...
### Parametric boorstrapping for standard errors
set.seed(123)
...
### Additional Mixtools capabilities
