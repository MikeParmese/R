# The Affect of McDonalds Press Release Sentiment on MCD Stock Price
...
#Navigate to McDonalds website
remDr$navigate("https://news.mcdonalds.com/press/press-releases/corporate-news")
...
for (i in 1:np) {
  ...
  for(j in 2:11){
    ...
    #page load error with encripted articles
    #code to skip these articles
...
#last page for loop returns "Error in el[[j]] : subscript out of bounds"
...
#loop uses list of pulled dates and and assignes them to the to and from date used by quantmod
for(i in 1:num){
  date1 <- stock.dates[i]
  if(i < num){
...
#make data frame from previous work
